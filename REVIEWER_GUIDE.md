...
```bash
python3 scripts/rebuild_research_artifacts.py
python3 scripts/verify_research_release.py
```

The rebuild command expects local ignored HFT quote/build outputs for build-dependent micro-alpha reports; the verifier checks committed artifacts only.
...
- The final micro-alpha research-quality scorecard has 8 pass / 3 warn / 1 fail, with three positive chronological folds, daily PSR sanity checks, and the only fail assigned to missing broker fill reconciliation.
